    ...
    if ($_GET['action'] == 'delete') {
        try {
            ...
            if ($result['booking_count'] > 0) {
                $_SESSION['error'] = "Không thể xóa chuyến đi vì đã có đặt vé.";
            } else {
        ...
        // Lấy status từ URL: 'scheduled' (Mở vé) hoặc 'paused' (Tạm dừng)
        ...
        $_SESSION['message'] = "Đã cập nhật trạng thái vé!";
        header("Location: manage_trip.php");
        exit();
    }
}
...
// --- XỬ LÝ 4: TỰ ĐỘNG CẬP NHẬT TRẠNG THÁI (ĐÃ SỬA LỖI GHI ĐÈ) ---
foreach ($trips as $trip) {
    $current_time = date('Y-m-d H:i:s');
    $departure_time = $trip['departure_time'];

    // --- [QUAN TRỌNG] CHỐT CHẶN BẢO VỆ ---
    // Nếu trạng thái là 'paused' (Tạm dừng) hoặc 'cancelled' (Đã hủy)
    // -> BỎ QUA NGAY, không chạy logic bên dưới để tránh bị ghi đè lại thành 'scheduled'
    if ($trip['status'] == 'paused' || $trip['status'] == 'cancelled') {
        continue;
    }

    // Logic tự động chỉ chạy cho vé đang mở hoặc đang full:
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $stmt = $pdo->prepare("
        SELECT t.*, dp.name as departure_province_name, dsp.name as destination_province_name 
        FROM trips t
        JOIN provinces dp ON t.departure_province_id = dp.id
        JOIN provinces dsp ON t.destination_province_id = dsp.id
        WHERE t.id = ?
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if ($trip['status'] == 'scheduled') {
                                        ...
                                        // QUAN TRỌNG: Nếu là 'paused' HOẶC rỗng (do lỗi cũ) thì đều hiện Tạm dừng
