...
if ($schedule_id <= 0 && empty($temp_id)) {
    die("Mã lịch trình hoặc thông tin tạm thời không hợp lệ.");
}
...
} elseif (!empty($temp_id) && isset($_SESSION['random_routes'][$temp_id])) {
...
if (!$trip) {
    ...
    die("Không tìm thấy thông tin chuyến đi. Vui lòng kiểm tra lại hoặc liên hệ admin.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($schedule_id > 0) {
            ...
            if ($trip['transport_type'] == 'xe buýt') {
                ...
                while ($booked_row = $booked_result->fetch_assoc()) {
                    ...
                    foreach ($seats as $seat) {
                        if (preg_match('/^(\d+)/', $seat, $matches)) { // For bus, seat format is just "number"
                ...
                for ($i = 1; $i <= $total_seats && $assigned < $quantity; $i++) {
                    if (!in_array($i, $booked_seats)) {
                        $seat_numbers[] = (string)$i; // For bus, just store the seat number
                        $assigned++;
                    }
                }
        ...
        if ($trip['transport_type'] == 'xe buýt') {
            $total_seats = 25; // Mặc định xe buýt có 25 ghế
            $available_seats = $total_available_seats - $quantity; // Trừ số ghế vừa đặt
            $total_carriages = 0; // Xe buýt không có toa
            $stmt->bind_param("isii", $route_id, $random_route['departure_time'], $total_carriages, $available_seats); 
        } else {
            $total_carriages = $random_route['is_round_trip'] ? count($carriage_list) : 1; // Số toa = số toa trong session (nếu là khứ hồi)
            $available_seats = 0; // Số ghế trống được tính ở bảng train_carriages
            $stmt->bind_param("isii", $route_id, $random_route['departure_time'], $total_carriages, $available_seats);
        }
        $stmt->execute();
        $schedule_id = $conn->insert_id; // Lấy schedule_id vừa tạo
        ...
        $stmt->bind_param("iisids", $_SESSION['user_id'], $schedule_id, $ticket_type, $quantity, $price, $seat_numbers_str);
        $stmt->execute();
        unset($_SESSION['random_routes'][$temp_id]); // Clean up session data
        header("Location: my-tickets.php?success=1&seats=" . urlencode($seat_numbers_str));
        exit();
    }
}
